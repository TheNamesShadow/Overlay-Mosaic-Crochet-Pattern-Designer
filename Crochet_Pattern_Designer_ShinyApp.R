...
ui <- fluidPage(
        ...
        div(
                ...
                h1("Create Your Very Own Overlay Mosaic Crochet Patterns!",
                   style = "font-size: 36px; font-weight: bold;"),
                h4("Just Upload an Image and Click 'Generate Pattern'",
                   style = "color: gray;")
        ),
        sidebarLayout(
                sidebarPanel(
                        ...
                        sliderInput("PatNumSlider", 
                                    label = h3("Split Instructions into ___ Patterns"), 
                                    min = 1, 
                                    max = 5, 
                                    value = 3),
                        
                        textInput("color1", label = h3("First Color (word or hex code)"), value = "black"),
                        textInput("color2", label = h3("Second Color (word or hex code)"), value = "white"),
                ...
                mainPanel(
                        tabsetPanel(
                                tabPanel("Overview", withSpinner(plotOutput("processed_image"))),
                                tabPanel("Pattern 1", withSpinner(plotOutput("Pattern_1"))),
                                tabPanel("Pattern 2", withSpinner(plotOutput("Pattern_2"))),
                                tabPanel("Pattern 3", withSpinner(plotOutput("Pattern_3"))),
                                tabPanel("Pattern 4", withSpinner(plotOutput("Pattern_4"))),
                                tabPanel("Pattern 5", withSpinner(plotOutput("Pattern_5")))
                        )
                )
        )
)
server <- function(input, output) {
        ...
        output$processed_image <- renderPlot({
                ...
                ggplot(image_df, aes(col, row, fill = color)) +
                        ...
                        scale_x_reverse() +
                        scale_fill_manual(values = c("#ffffffff" = input$color2, "#000000ff" = input$color1)) +
